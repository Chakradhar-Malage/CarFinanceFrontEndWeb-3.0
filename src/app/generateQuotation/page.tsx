...
"use client"; // Ensures it's a client-side component in Next.js
...
const GenerateQuotation = () => {
  ...
  useEffect(() => {
    axios
      .get("http://72.61.245.17:3000/quotation-customers")
  ...
  const generateInvoice = async () => {
    try {
      const response = await axios.post("http://72.61.245.17:3000/generate-quotation", {
      ...
      {isNewCustomer ? (
        <Card className="mb-6 p-4">
          <CardContent>
            ...
            {["name", "mobile", "address", "gstin"].map((field) => (
              <TextField
                key={field}
          ...
          {selectedCustomer && (
            <Card className="mb-6 p-4">
              <CardContent>
                ...
                {["name", "mobile", "address", "gstin", "termsConditions"].map((field) => (
                  <TextField
                    key={field}
                    label={`Customer ${field.charAt(0).toUpperCase() + field.slice(1)}`}
                    value={customerDetails[field as keyof typeof customerDetails]}
